...
/*     (`if (len && len <= i) break;`, uart.c:110) - the old bt_recv passed */
/*     0 into a 64-byte stack buffer - and it CLEARS UART_MCR_RTS for the   */
/*     duration of the read (uart.c:101-106), inverted-RTS behaviour that   */
/*     exists for the Joy-Con link and would undo our RTS assertion         */
/*     precisely while we listen. Nothing below calls either function.      */
/*                                                                          */
...
#define BT_CFG_WL_LOW      1   /* drive PH1/WL_REG_ON low for a cold POR      */
#define BT_CFG_LPO_OUT1    1   /* mux MAX77620 GPIO4 to 32K_OUT1 (vendor)     */
#define BT_CFG_PORT_I_SPIO 1   /* force PI4..PI7 out of GPIO mode             */
...
/* PINMUX_AUX offsets. The "0x1B4 + 4n" rule the old comment relied on is
 * only valid to PH5 - BDK's own header proves it: PINMUX_AUX_AP_WAKE_NFC is
 * 0x1CC (that pad is PH7, which the Switch reuses as BT_GPIO5) and
 * PINMUX_AUX_GPIO_PH6 is 0x250, nowhere near the run. PK0..PK2 are derived
 * backwards from BDK's PINMUX_AUX_GPIO_PK3, so they cannot drift either. */
 ...
 * read-modify-writes over the whole port register (gpio.c:57-91) - a single
 * stray RMW that catches bit 5 while GPIO_OUT bit 5 is clear drives
 * BT_HOST_WAKE hard low at full drive strength, beats the chip's internal
 * pull-up, and latches SPI transport for the entire power cycle. */
...
/* Park every pad the CYW4356 can sample, with BT_REG_ON (and optionally
 * WL_REG_ON) driven LOW. Whole-value pinmux writes, never OR, so PINMUX
 ...
 *   already had to do for UART-B RX in ipl_main. So: pull-down for the
...
static void bt_uart_pads(u32 rx_pull)
{
     ...
     * handback for UART-B/C in hw_init (hw_init.c:458-462) and this tree had
     * to add it for UART-B RX in ipl_main, but there is no UART-D case
...
static bool bt_attempt(const bt_arm_t *arm, u8 *ev, u32 ev_max, u32 *ev_n)
{
     ...
     *    choosing not to answer - a completely different bug that used to
    ...
    /* 11. H4 HCI_Reset, opcode 0x0C03, no parameters. Unconditionally the
     *     first command on the wire for every Broadcom UART part, in normal
     *     and autobaud mode alike (btbcm.c:582-585 -> :272-289). No firmware
     *     download is needed first: the part runs its lower-layer stack from
     *     668 KB of on-die ROM (p27 s6) and Horizon pushes its patchram
     *     in-band over an already-working HCI link.
     *
    ...
    static const u8 hci_reset[] = { 0x01, 0x03, 0x0C, 0x00 };
    *ev_n = 0;
    for (u32 t = 0; t < BT_CFG_HCI_TRIES && !*ev_n; t++) {
        if (!bt_send(hci_reset, sizeof(hci_reset))) {
    ...
    bool cc = (*ev_n >= 7 && ev[1] == 0x0E && ev[4] == 0x03 &&
               ev[5] == 0x0C && ev[6] == 0x00);
...
static void probe_bt_radio(void)
{
    volatile uart_t *u = BT_UART_REGS;
    u8  ev[72];
    u32 ev_n = 0;
    ...
    HEADER("[Bluetooth radio (CYW4356 HCI over UART-D)]");

    /* Entry state, before anything is touched. This is the one read that
     * settles the "the BootROM left the Tegra driving BT_HOST_WAKE low"
     * hypothesis outright: OE bit 5 set with OUT bit 5 clear is the
     * SPI-strap condition, and it has never once been measured. */
    ...
    /* Rails. Still brute force at their existing voltages: we have not
     * identified which supply reaches BT_VDDIO through the board's load
     * switch, and no Switch device tree gives the bluetooth node any
     * regulator, clock or pwrseq property to point at one. Diagnostic step,
     * not an order Horizon ever uses. */
     ...
     * plain GPIO and no clock leaves the pin. 32K_OUT0 (CNFG1_32K bit 2),
     * which this probe used to check, feeds the Tegra's own clk_32k_in.
     *
...
#if BT_CFG_BAUD_SWEEP
    /* A powered, out-of-POR part in UART transport that simply runs at a
     * non-standard rate produces byte-identical symptoms to a dead one.
     * Kernel commit af35e28f0fea says outright that autobaud exists partly
     * because "the controller device is using a non-standard baud rate in
     * normal mode when first powered on", and p38 Table 15 gives the chip's
     * own rate list. Cheapest experiment that separates the two. */
    if (!ok) {
        static const u32 bauds[] = { 921600, 460800, 230400, 57600,
                                     38400, 19200, 9600, 3000000 };
        static const u8 hci_reset[] = { 0x01, 0x03, 0x0C, 0x00 };
        log_color(COL_INFO, "  -- host baud sweep --\n");
        for (u32 i = 0; i < ARRAY_SIZE(bauds) && !ok; i++) {
            u8 raw[32]; u32 raw_n = 0;
            bt_uart_cfg(bauds[i], BT_CFG_MCR_HCI);
            bt_flush();
            if (!bt_send(hci_reset, sizeof(hci_reset)))
                continue;
            ev_n = bt_wait_event(ev, sizeof(ev), 300, raw, sizeof(raw),
                                 &raw_n);
            ok = (ev_n >= 7 && ev[1] == 0x0E && ev[4] == 0x03 &&
                  ev[5] == 0x0C && ev[6] == 0x00);
            LOG("  %7d bps  : ev=%d raw=%d %s\n",
                bauds[i], ev_n, raw_n, ok ? "COMMAND COMPLETE" : "-");
            if (raw_n)
                bt_dump(raw, raw_n);
        }
        if (!ok)
            bt_uart_cfg(115200, BT_CFG_MCR_HCI);
    }
#endif
     ...
     * The old code read manufacturer at [10..11], one byte early, which
     * yields lmp_ver | (manuf_lo << 8) = 0x0F08 and prints "not Broadcom?"
     * on a perfectly good chip.
     *
...
static const char *_k_storage[] = { "emmc_health", "emmc_bus", "sd_bus",
                                    ...
                                    "sd_errors", NULL };
static const char *_k_wireless[] = { "bt_hci", NULL };
