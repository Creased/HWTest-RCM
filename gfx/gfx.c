...
/* hwtest renders in LANDSCAPE: the LCD is physically 720 wide x 1280 tall
 * (portrait) but we treat it as a 1280 x 720 canvas and rotate every pixel
 * write 90 deg CW. Hold the console with the right Joy-Con rail at the
 * top (UART cable goes up) and the text reads naturally.
 *
 * Mapping: logical (lx, ly), lx in [0..1279], ly in [0..719] ->
 *          physical fb index (ly) + (1279 - lx) * 720
 *
 * gfx_ctxt is therefore set up with width=1280, height=720, stride=720
 * (stride is unchanged: it's still the physical row stride, but the
 * landscape-aware pixel writes ignore it and use the rotation formula).
 *
 * Glyph rendering goes through _gfx_set_lpixel for clarity — slower than
 * the original pointer-incrementing loop but bandwidth is fine for a
 * console used for diagnostic text. */
#define GFX_PHYS_W 720u
#define GFX_PHYS_H 1280u

static inline void _gfx_set_lpixel(u32 lx, u32 ly, u32 color)
{
    if (lx >= GFX_PHYS_H || ly >= GFX_PHYS_W) return;
    gfx_ctxt.fb[ly + (GFX_PHYS_H - 1u - lx) * GFX_PHYS_W] = color;
}
...
{
	/* Landscape variant: glyphs render via _gfx_set_lpixel which applies
	 * the 90 deg CW rotation. We keep the 16 / 8 size split for parity
	 * with the upstream gfx but use a generic pixel writer rather than
	 * pointer-arithmetic optimisations — performance is plenty for the
	 * couple of KB of diagnostic text we paint per page redraw. */
	...
	case 16:
		if (c >= 32 && c <= 126)
		{
			...
			{
				u8 v = cbuf[row >> 1];           /* same byte for two consecutive rows */
				for (u32 col = 0; col < 16; col++)
				{
					bool on = (v >> (col >> 1)) & 1;
					if (on)
						_gfx_set_lpixel(gfx_con.x + col, gfx_con.y + row, gfx_con.fgcol);
					else if (gfx_con.fillbg)
						_gfx_set_lpixel(gfx_con.x + col, gfx_con.y + row, gfx_con.bgcol);
				}
			}
			gfx_con.x += 16;
		}
