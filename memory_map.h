...
/* HWTest-RCM fork: the SDRAM extraction buffer sits at 0x40036000, above
 * the upstream address, so both shipped variants (including the Joy-Con
 * build with the PCIe probe) fit below it. The rest of this file is
 * byte-identical to the vendored BDK memory_map.h - see
 * third_party/hekate/bdk/memory_map.h. Keep the two in sync except for
 * this one line. */
#define SDRAM_PARAMS_ADDR 0x40036000 // SDRAM extraction buffer during sdram init.
